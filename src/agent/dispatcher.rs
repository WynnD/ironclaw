...
mod tests {
    ...
    /// Simulates a native GLM-style provider response that includes structured
    /// tool calls with preserved thinking content.
    struct NativeStyleToolCallProvider;

    #[async_trait]
    impl LlmProvider for NativeStyleToolCallProvider {
        fn model_name(&self) -> &str {
            "zai-org/GLM-5"
        }
        ...
        async fn complete_with_tools(
            ...
            Ok(ToolCompletionResponse {
                content: Some(
                    "I should run a command.<thinking>native reasoning</thinking>".to_string(),
                ),
                tool_calls: vec![ToolCall {
                    id: "native_call_1".to_string(),
    ...
    #[tokio::test]
    async fn test_native_style_tool_calls_still_require_approval() {
        use crate::channels::IncomingMessage;
        use crate::llm::{ChatMessage, Role};
        use crate::tools::builtin::shell::ShellTool;
        use tokio::sync::Mutex;

        let llm: Arc<dyn LlmProvider> = Arc::new(NativeStyleToolCallProvider);
        let agent = {
            let deps = AgentDeps {
                store: None,
                llm,
                ...
                tools: {
                    let registry = Arc::new(ToolRegistry::new());
                    registry.register_sync(Arc::new(ShellTool::new()));
                    registry
                },
                workspace: None,
                extension_manager: None,
        ...
        let message = IncomingMessage::new("test", "test-user", "clean temp files");
        let initial_messages = vec![ChatMessage::user("clean temp files")];

        let result = agent
            .run_agentic_loop(&message, session, thread_id, initial_messages)
            .await
            .expect("dispatcher should return a loop result");

        match result {
            super::AgenticLoopResult::NeedApproval { pending } => {
                assert_eq!(pending.tool_name, "shell");
                assert_eq!(pending.tool_call_id, "native_call_1");
                assert_eq!(pending.parameters["command"], "rm -rf /tmp/test");
                assert!(
                    pending
                        .context_messages
                        .iter()
            ...
            super::AgenticLoopResult::Response(text) => {
                panic!("Expected NeedApproval for shell tool call, got response: {text}");
            }
        }
    }
}
